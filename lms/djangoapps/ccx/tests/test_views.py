...
class TestCCXGrades(FieldOverrideTestMixin, SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_grades_csv(self):
        ...
        response = self.client.get(url)
        assert response.status_code == 200
        # The CSV downloads as an attachment with an Instructor-Dashboard-style
        # filename: <course_prefix>_grade_report_<timestamp>.csv
        ...
        headers = rows[0]
        # Identity/grade column titles match the Instructor Dashboard report.
        assert headers.strip().split(',')[:4] == ['Student ID', 'Email', 'Username', 'Grade']
    ...
    def test_grades_csv_skips_ungraded_students(self):
        """
        Students the grading machinery could not grade are skipped rather than
        emitted as broken rows.
        """
        self.course.enable_ccx = True
        RequestCache.clear_all_namespaces()
        ...
        # CourseGradeFactory.iter yields a null grade plus an error message when
        # a student cannot be graded.
        grade_results = [
            (self.student, None, 'Error grading student'),
            (self.student2, None, 'Error grading student'),
        ]
        with patch.object(CourseGradeFactory, 'iter', return_value=iter(grade_results)):
            response = self.client.get(url)

        assert response.status_code == 200
        body = response.content.decode('utf-8')
        # No rows at all are emitted, not even a header, and neither student leaks
        # into the report.
        assert body.strip() == ''
        assert self.student.email not in body
        assert self.student2.email not in body
